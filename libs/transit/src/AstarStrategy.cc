...
AstarStrategy::AstarStrategy
    ...
    std::vector<float> start = graph->NearestNode
        (positionV, EuclideanDistance())->GetPosition();
    std::vector<float> end = graph->NearestNode
        (destinationV, EuclideanDistance())->GetPosition();
...
void AstarStrategy::Move(IEntity* entity, double dt) {
    ...
    Vector3 destination = Vector3
        (path[currentIndex].at(0), path[currentIndex].at(1),
